...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('student_id', 'Full_name', 'contact','display_image','active', 'block_id','room_id');
		...
		{
			$search_query .= 'AND ( student_id LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR contact LIKE "%'.$_POST["search"]["value"].'%" ';
		
			$search_query .= 'OR Full_name LIKE "%'.$_POST["search"]["value"].'%") ';
		}
		...
		{
			$sub_array = array();
			$sub_array[] = '<img src="'.$row["display_image"].'" class="img-fluid img-thumbnail" width="75" height="75" />';
			$sub_array[] = html_entity_decode($row["admin_name"]);
			$sub_array[] = $row["student_id"];
			$sub_array[] = $row["Full_name"];
			$sub_array[] = $row["contact"];
			$sub_array[] = $row["active"];
			$sub_array[] = $row["block_id"];
	...
	if($_POST["action"] == 'Add')
	{
		...
		{
			$error = '<div class="alert alert-danger">student id Already Exists</div>';
		}
		else
		{
			$user_image = '';
			...
			{
				$user_image = make_avatar(strtoupper($_POST["Full_name"][0]));
			}

			$data = array(
				':student_id'		=>	$visitor->clean_input($_POST["student_id"]),
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		foreach($result as $row)
		{
			...
			$data['contact'] = $row['active'];
			$data['contact'] = $row['block_id'];
			$data['contact'] = $row['room_id'];
			$data['display_image'] = $row['display_image'];
		}
	...
	if($_POST["action"] == 'Edit')
	{
		...
		else
		{
			...
			$data[':student_id'] = $visitor->clean_input($_POST["student_id"]);
			$data[':Full_name'] = $_POST["Full_name"];
			$data[':contact'] = $_POST["admin_contact_no"];
